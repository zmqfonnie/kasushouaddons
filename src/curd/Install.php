...
class Install extends Command
{
    ...
    protected function execute(Input $input, Output $output)
    {
        ...
        $this->sqlFile = app()->getBasePath() . "install/kasushou.sql";
...
//        $this->output->info('fileinfo extension is installed');
        ...
        $sql_file = app()->getBasePath().'install'.DIRECTORY_SEPARATOR.'kasushou.sql';
        //检测能否读取安装文件
    ...
    protected function install($input): void{
        ...
        $db['database'] = strtolower($this->output->ask($this->input, '👉 Set mysql database default (kasushou)'))?:$db["database"];
        $db['prefix'] = strtolower($this->output->ask($this->input, '👉 Set mysql table prefix default ( xinmeng_)'))?:$db["prefix"];
...
//            $link->query("set global max_allowed_packet=104857600");
            ...
            $sql = str_replace(["` xinmeng_",'CREATE TABLE'], ["`{$db['prefix']}",'CREATE TABLE IF NOT EXISTS'], $sql);
