...
\think\Console::starting(function (\think\Console $console) {
    $console->addCommands([
        'addons:config' => '\\kasushou\\addons\\command\\SendConfig',
        'auth:config' => '\\kasushou\\auth\\command\\SendConfig',
        'curd:config' => '\\kasushou\\auth\\command\\SendConfig'
    ]);
});
...
if (!function_exists('addons_route_autoload')) {
    /**
     * 加载插件路由
     * @params mixed $addonsName 插件名称或插件数组
     */
...
//        clog([], 'addons_route_autoload');
        ...
        foreach ($routes as $key => $val) {
            ...
            if (is_array($val)) {
                foreach ($val as $k => $rule) {
                    ...
                    Route::{$rule['method']}($rule['key'], $execute)
                        ->name($rule['name'] ?? '插件路由')
                        ->completeMatch(true)
        ...
        foreach (get_addons_list() as $item) {
            $addon_path = app()->addons->getAddonsPath() . $item['name'] . '/';
            // 注册插件中间件
            ...
            //插件路由
            $route_file = $addon_path . 'route.php';
            if (is_file($route_file)) {
//                Route::post('addon/payreg/status', "\addons\payreg\controller\Index::status")->name('验证付费支付状态');
//                Route::post('addon/payreg/config', "\addons\payreg\controller\Index::config")->name('获取付费配置');
                include $route_file;
            }
    ...
    function get_addons_autoload_config($chunk = false)
    {
        ...
        foreach ($addons as $name => $addon) {
            ...
            if ($conf) {
                ...
                $rule = $conf['rewrite'] ? $conf['rewrite'] : [];
                if ($rule) {
                    $route = array_merge($route, [$name => $rule]);
                }
...
if (!class_exists('Form')) {
    class_alias('kasushou\\Form', 'Form');
}
